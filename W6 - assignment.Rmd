...
**Please note that as i have changed the column entity to country in task 4, it have changed thoughout the entire assignment**

I use the function 'names' flowed by the name of the name of the dataset and then the index of the column it want changed, which in this case is index 4 - and then changing the name to 'homicides'. 
```{r}
...
For this one, i found that the function geom_line makes the visualization a lot easier to read when in comes to the lon term data. Mainly because of the fact, that it adds the line, which makes it more readable when trying to understand the development of the graph/data. 

```{r}
data <- (data = Western_Europe)
data %>%
  tail(10) %>%
...
With the function facet_wrap i am able to customize the rows and columns of the graphs shown. As seen below i've set the rows = and columns = 2 as asked for in the task. 
...
For this task i have used the function names as in the first task to change the name of the first column Entity to Country. 
Furthermore i have made use of the function theme(legend.position) to move the legend from the side of the chart to the bottom. 
...
See header and html-doc in repository
