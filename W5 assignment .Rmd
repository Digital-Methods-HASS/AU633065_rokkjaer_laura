...
#by using this line of code, where I filter the dataset by looking for the year that macthes 1952, we can se that Kuwait is the richest contry in 1952
gapminder %>% 
  filter(year == 1952) %>% 
...
#down belove you can see the code for differentiating the continents by color, where i make the color differentiate by whatever continent it has (color =  continent)
#futhermore i have also used the labs method in order to make the labels more legible as well as the units
...
#this is the same methods as in the above tasks, just with the 5 richest, where i used the function, head(5)
gapminder %>% 
  filter(year == 2007) %>% 
  arrange(desc(gdpPercap)) %>% 
  head(5)
...
#i wanted to make the animations transition from the years elvolving. Both the transition_time and transition_states where used to make this work. 
...
#To make the axes more readable i used the function labs, as in some of the above tasks
anim2 <- ggplot(gapminder, aes(gdpPercap, lifeExp, size = pop)) +
  geom_point() +
  scale_x_log10() + # convert x to log scale
  transition_time(year) +
